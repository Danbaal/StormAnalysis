...
In the first decade of this century, weather events hasve caused `r format(total_fatalities, nsmall=0)` fatalities and `r format(total_injuries, nsmall=0)` injuries, resulting in a total of `r format(total_casualties, nsmall=0)` casualties. We can see the big picture about the damage each event have caused to the population health in de Health Damage graphic. Turns out that tornados are the most harmful weather event to the population health. They has caused 1152 fatalities and 14331 injuries, representing the `r round(tornado_c_stat*100, digits = 2)`% of the total casualties caused by weather events in this century. Heat also have a big impact on the health population, causing `r round(heat_c_stat*100, digits = 2)`% of the total casualties. Other weather events worth to mention because of its severity are thunderstorm winds, lightning and floods.

Speaking about the economic impact, weather events have caused losses of $`r round(total_prop, digit=2)` million to properties and $`r round(total_crop, digit=2)` million in crops, resulting in a total of $`r round(total_econ_loss, digit=2)` million in losses. As we can see in the Economic Damage graphic showed before, floods are the most harmful to the ecomomy causing $`r round(flood_total, digit=2)` million in losses and representing the `r round(flood_e_stat, digit=2)`% of the total loss caused by weather events.
