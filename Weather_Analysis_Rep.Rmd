...
thtmwind_c_stat <- (casual_table[3,2] + casual_table[3,3])/total_casualties
...
So far this century, weather events has caused `r format(total_fatalities, nsmall=0)` fatalities and `r format(total_injuries, nsmall=0)` injuries, resulting in a total of `r format(total_casualties, nsmall=0)` casualties. Turns out that tornados are the most harmful weather event to the population health. They has caused 1152 fatalities and 14331 injuries, representing the `r round(tornado_c_stat*100, digits = 2)`% of the total casualties caused by weather events in this century. After the tornadoes, the second place, in total number of casualties, comes the events related with a excessive heat...  Weather events that cause an excessive heat are the second most harmful to the population health, they cause `r ` less casualties than tornados, but have caused almost the same number of fatalities. Otro evento que destaca por el numero de muertes es flood,. Sumarizing casualties,